...
def apply_review_classification():
   ...
   #
    ...
    classification_df = pd.DataFrame({'Review Classification': review_classification})
...
# Add a new function for CNN classification on selected data
def apply_cnn():
    # Display operation begin label in red
    begin_label.config(text="Performing CNN Classification on Selected Data", fg="red")
    root.update()
    ...
    selected_df['Predicted_Classification'] = selected_df.apply(lambda row: predict_sentiment(row), axis=1)

    # Tokenize and pad the text data
    max_len = 100  # Adjust as needed
    vocab_size = 5000  # Adjust as needed
    tokenizer = Tokenizer(num_words=vocab_size, oov_token='<OOV>')
    tokenizer.fit_on_texts(selected_df['Predicted_Classification'])
    sequences = tokenizer.texts_to_sequences(selected_df['Predicted_Classification'])
    ...
    for text_id, selected_text in zip(text_id, selected_text):
        predicted_class = predict_sentiment({'Text': selected_text})
    ...
    print('CNN results saved to cnn.csv')
...
# Function to predict sentiment based on the given logic
def predict_sentiment(row):
    sid = SentimentIntensityAnalyzer()
    sentiment_score = sid.polarity_scores(str(row['Text'])) if pd.notna(row['Text']) else {}
...
# Add a new function for Random Forest classification on selected data
def apply_random_forest_classification():
    # Display operation begin label in red
    begin_label.config(text="Performing Random Forest Classification on Selected Data", fg="red")
    root.update()
    ...
    # Apply Random Forest classification to each row separately
    sid = SentimentIntensityAnalyzer()
    ...
    for text_id, selected_text in zip(text_id, selected_text):
        ...
        if sentiment_score['compound'] >= 0.05:
            ...
            classification = 'Neutral'
        random_forest_classification.append(
            f"Text: {selected_text}\nRandom Forest Classification: {classification}\n\n"
        )

    # Display the classification results in the text box
    classification_text = '\n\n'.join(random_forest_classification)
    text_area.delete(1.0, tk.END)
    text_area.insert(tk.END, classification_text)

    # Write classification results to a CSV file
    random_forest_df = pd.DataFrame({'Random_Forest_Classification': random_forest_classification})
    random_forest_df.to_csv('random_forest_classification_output.csv', index=False)
    print('Random Forest classification results saved to random_forest_classification_output.csv')

    # Display operation end label in green
    begin_label.config(text="Random Forest Classification on Selected Data operation Complete", fg="green")
    root.update()
...
#btn10 = Button(root, text="11. FUZZYFICATION", fg="white", bg="blue", activebackground="red", command=apply_random_forest_classification)
