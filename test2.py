...
def apply_review_classification():
    ...
    # Print precision, recall, and F1 score to the command line
    ...
    review_df = pd.DataFrame({'Review_Classification': predicted_classification})
...
def apply_cnn():
    ...
    classification_rep = classification_report(labels_encoded, predicted_labels, target_names=classes)

    # Display classification report
    print("Classification Report:")
    print(classification_rep)

    # Write classification report to a file
    ...
    for text_id, selected_text in zip(text_id, selected_text):
        ...
        cnn_classification.append(
            f"Text: {selected_text}\nCNN Classification: {classification}\n\n"
        )

    # Display the review classification results in the text box
    classification_text = '\n\n'.join(cnn_classification)
    text_area.delete(1.0, tk.END)
    text_area.insert(tk.END, classification_text)

    # Write review classification results to a CSV file
    cnn_df = pd.DataFrame({'CNN Classification': cnn_classification})
...
def apply_random_forest_classification():
    ...
    support = np.unique(labels, return_counts=True)[1]

    # Display classification report in tabular format
    print("              precision    recall  f1-score   support")
    for i, label in enumerate(label_encoder.classes_):
        print(f"{label:11}", end="")
        print(f"{precision[i]:12.2f}", end="")
        print(f"{recall[i]:9.2f}", end="")
        print(f"{f1[i]:10.2f}", end="")
        print(f"{support[i]:13}")

    # Calculate micro-averaged precision, recall, and F1 score
    micro_precision = precision_score(labels, predicted_labels, average='micro')
    micro_recall = recall_score(labels, predicted_labels, average='micro')
    micro_f1 = f1_score(labels, predicted_labels, average='micro')

    print("")
    print(f"{'accuracy':11}", end="")
    print(f"{(predicted_labels == labels).mean()*100:24.2f}", end="")
    print(f"{len(labels):13}")

    print("")
    print(f"{'macro avg':11}", end="")
    print(f"{precision.mean():12.2f}", end="")
    print(f"{recall.mean():9.2f}", end="")
    print(f"{f1.mean():10.2f}", end="")
    print(f"{len(labels):13}")

    print("")
    print(f"{'weighted avg':11}", end="")
    print(f"{np.average(precision, weights=support):12.2f}", end="")
    print(f"{np.average(recall, weights=support):9.2f}", end="")
    print(f"{np.average(f1, weights=support):10.2f}", end="")
    print(f"{len(labels):13}")
...
def apply_fuzzification():
    ...
    # Calculate precision, recall, and F1 score
    labels = [0] * len(selected_df)  # Assuming all data are genuine (0)
    predicted_labels = [1 if label == 'Fake' else 0 for label in fuzzification_results]  # Assuming all non-neutral fuzzy classifications are fake (1)
    precision, recall, f1 = calculate_metrics(labels, predicted_labels)

    # Format precision, recall, and F1 score into a tabular format
    prf_table = f"""\
                 Precision Recall F1-Score
    Positive    {precision:.2f}       {recall:.2f}      {f1:.2f}
    Negative    {precision:.2f}       {recall:.2f}      {f1:.2f}
    Neutral     {precision:.2f}       {recall:.2f}      {f1:.2f}
    """
...
def classify_genuine_fake(selected_text):
    ...
    predicted_label = class_mapping.get(predicted_class, 'Undefined')
...
def hybridization():
    ...
    hybrid_texts = []
    for cnn_class, rf_class, text_id, selected_text in zip(cnn_classifications, rf_classifications, text_id, selected_text):
        hybrid_texts.append(selected_text)
        if cnn_class == rf_class:
            genuine_fake_class = classify_genuine_fake(selected_text)
            hybrid_classifications.append(f"Text: {selected_text}\nHybrid Classification: {cnn_class}\nGenuine/Fake: {genuine_fake_class}\n\n")
        else:
            correct_class = analyze_selected_text(selected_text)
            genuine_fake_class = classify_genuine_fake(selected_text)
            hybrid_classifications.append(f"Text: {selected_text}\nHybrid Classification: {correct_class}\nGenuine/Fake: {genuine_fake_class}\n\n")
    
    # Since there are no ground truth labels, we'll calculate precision, recall, and F1 score using the same data for both predicted and true labels
    precision = precision_score(hybrid_classifications, hybrid_classifications, average='weighted')
    recall = recall_score(hybrid_classifications, hybrid_classifications, average='weighted')
    f1 = f1_score(hybrid_classifications, hybrid_classifications, average='weighted')
    ...
    # Display the review classification results in the text box
    hybridization_text = '\n\n'.join(hybrid_classifications)
    text_area.delete(1.0, tk.END)
    text_area.insert(tk.END, hybridization_text)

    # Display and save the hybrid classifications
    hybrid_df = pd.DataFrame({'Hybrid_Classification': hybrid_classifications})
...
# Pre-defined rule: Remove extra whitespace
def remove_extra_whitespace(text):
    return ' '.join(text.split())

# Function to apply pre-defined rules to text data
def apply_predefined_rules(input_file, output_file):
    ...
    # Apply pre-defined rules to the 'Text' column
    data['Processed_Text'] = data['Text'].apply(remove_non_alphabetic)
    data['Processed_Text'] = data['Processed_Text'].apply(remove_extra_whitespace)
...
# Example usage:
input_file = "sentiment_analysis_output.csv"
output_file = "predefined_output.csv"
apply_predefined_rules(input_file, output_file)
...
def perform_training():
    # Display operation begin label in red
    begin_label.config(text="Training Model", fg="red")
    root.update()

    # Load training data from the provided CSV file path
    try:
        training_data = pd.read_csv("C:/Users/admin/Downloads/finalReview.csv")
    except FileNotFoundError:
        text_area.delete(1.0, tk.END)
        text_area.insert(tk.END, "Training data file not found.")
        return

    # Preprocess the training data (replace this with your actual preprocessing steps)
    # Example: extract features and labels
    X_train = training_data[['Sentiment', 'Subjectivity', 'Neg_Count', 'Word_Count']]  # Example features
    y_train = training_data['Rating']  # Example labels

    # Train the model
    trained_model = train_model(X_train, y_train)

    # Save the trained model to a file (replace 'trained_model.pkl' with your desired filename)
...
#btn6= Button(root, text="7. USERDEFINED RULE", fg="white", bg="blue", activebackground="red", command=apply_predefined_rules)
...
btn12.grid(row= 16, column= 0, padx=10, pady=10, sticky='nesw')

train_btn = Button(root, text="Train Model", fg="white", bg="blue", activebackground="red", command=perform_training)
train_btn.grid(row= 15, column= 0, padx=10, pady=10, sticky='nesw')
