...
def apply_review_classification():
    ...
    ground_truth_df = pd.read_csv('C:/Users/Aniket/OneDrive/Desktop/sentiment analysis_15-01-2024/Review.csv')
    ...
    # Calculate metrics
    try:
        precision = precision_score(ground_truth_labels[:len(predicted_labels)], predicted_labels, average=None)
        recall = recall_score(ground_truth_labels[:len(predicted_labels)], predicted_labels, average=None)
        f1 = f1_score(ground_truth_labels[:len(predicted_labels)], predicted_labels, average=None)
        accuracy = accuracy_score(ground_truth_labels[:len(predicted_labels)], predicted_labels)
...
def apply_cnn():
    ...
    classification_rep = classification_report(ground_truth_labels[:len(predicted_labels)], predicted_labels, target_names=classes)
...
"""def apply_random_forest_classification():
    ...
    # Calculate precision, recall, F1 score, and accuracy based on ground truth
    accuracy = accuracy_score(ground_truth_labels, predicted_classification)
    precision = precision_score(ground_truth_labels, predicted_classification, average=None)
    recall = recall_score(ground_truth_labels, predicted_classification, average=None)
    f1 = f1_score(ground_truth_labels, predicted_classification, average=None)
...
def apply_random_forest_classification():
    ...
    # Calculate precision, recall, F1 score, and accuracy based on ground truth
    accuracy = accuracy_score(ground_truth_labels, encoded_predicted_labels)
    precision = precision_score(ground_truth_labels, encoded_predicted_labels, average=None)
    recall = recall_score(ground_truth_labels, encoded_predicted_labels, average=None)
    f1 = f1_score(ground_truth_labels, encoded_predicted_labels, average=None)

    # Display classification report in tabular format
    print("\nMetrics:")
    print("              precision    recall  f1-score    accuracy ")
    for label, prec, rec, f in zip(label_encoder.classes_, precision, recall, f1):
        print(f"{label:11}{prec:12.2f}{rec:9.2f}{f:10.2f} {accuracy:12.2f}")
...
def apply_fuzzification():
    ...
    predicted_labels = [1 if label == 'Fake' else 0 for label in fuzzification_results]

    # Calculate metrics based on ground truth
    accuracy = accuracy_score(ground_truth_labels_encoded, predicted_labels)
    precision = precision_score(ground_truth_labels_encoded, predicted_labels, average='weighted')
    recall = recall_score(ground_truth_labels_encoded, predicted_labels, average='weighted')
    f1 = f1_score(ground_truth_labels_encoded, predicted_labels, average='weighted')
    ...
    # Display the combined text in the textbox
    text_area.delete(1.0, tk.END)
    text_area.insert(tk.END, fuzzification_text)
...
def hybridization():
    ...
    # Convert ground truth labels to numerical labels
    ground_truth_df = pd.read_csv('C:/Users/Aniket/OneDrive/Desktop/sentiment analysis_15-01-2024/Review.csv')
    ground_truth_labels = ground_truth_df['Rev_Type']
    label_encoder = LabelEncoder()
    ground_truth_labels_encoded = label_encoder.fit_transform(ground_truth_labels)

    # Convert hybrid classifications to numerical labels
    hybrid_classifications_encoded = label_encoder.transform(hybrid_classifications)

    # Calculate precision, recall, and F1 score
    precision = precision_score(ground_truth_labels_encoded, hybrid_classifications_encoded, average='weighted')
    recall = recall_score(ground_truth_labels_encoded, hybrid_classifications_encoded, average='weighted')
    f1 = f1_score(ground_truth_labels_encoded, hybrid_classifications_encoded, average='weighted')
...
fg="black", bg="skyblue", width= 90, borderwidth=5, relief="groove", font =('Verdana', 15))
