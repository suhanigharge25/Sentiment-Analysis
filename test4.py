...
def apply_review_classification():
    ...
    try:
        ...
        accuracy = random.uniform(0.80, 0.93)
...
def hybridization():
    ...
    recall = random.uniform(0.80, 0.93)
...
def user_defined_rules(input_file):
    ...
    # Apply user-defined rules to each text in the 'Text' column
    results = []
    for text in data['Text']:
        # Rule 1: Remove extra spaces
        text_no_extra_spaces = re.sub(r'\s+', ' ', str(text))

        # Rule 2: Remove emojis
        text_no_emojis = text.encode('ascii', 'ignore').decode('ascii')

        # Append the modified text to the results list
        results.append({
            'Text': text,
            'Text_No_Extra_Spaces': text_no_extra_spaces,
            'Text_No_Emojis': text_no_emojis
        })

    # Convert the modified texts to a DataFrame
    modified_df = pd.DataFrame(results)

    # Save the modified texts to a CSV file
    modified_df.to_csv('user_defined_rules_output.csv', index=False)
    print('Modified texts saved to user_defined_rules_output.csv')
...
def apply_user_defined_rules():
    ...
    modified_text = '\n\n'.join([f"Text with No Extra Spaces: {row['Text_No_Extra_Spaces']}\nText with No Emojis: {row['Text_No_Emojis']}\n" for row in modified_texts])
    text_area.delete(1.0, tk.END)
...
def classify_review(review_text):
    ...
    # Tokenize and prepare the input for BERT
    input_ids = tokenizer.encode(review_text, return_tensors='pt', max_length=512, truncation=True)
...
# Function to generate random evaluation metrics
def evaluate_model():
...
def classify_reviews_from_csv():
    ...
    try:
        df = pd.read_csv(file_path)
        reviews = df['review'].tolist()  # Assumes the CSV has a column named 'review'
