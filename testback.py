...
def apply_review_classification():
    ...
    # Display the review classification results and metrics in the text box
    classification_text = '\n\n'.join([
    ...
    review_df = pd.DataFrame({'Review_Classification': predicted_classification, 'Precision': [precision]*len(predicted_classification), 'Recall': [recall]*len(predicted_classification), 'F1 Score': [f1]*len(predicted_classification)})
...
def apply_cnn(tokenizer):
    ...
    predicted_probabilities = cnn_model.predict(padded)

    # Convert predicted probabilities to binary labels
    predicted_labels = [1 if prob >= 0.5 else 0 for prob in predicted_probabilities]

    # Get the true labels (0 for 'Negative' and 1 for 'Positive')
    true_labels = [1 if label == 'Positive' else 0 for label in selected_df['Sentiment']]
    ...
    metrics_text = f"Precision: {precision}\nRecall: {recall}\nF1 Score: {f1}\n\n"
    text_area.delete(1.0, tk.END)
    text_area.insert(tk.END, metrics_text)

    # Write classification results to a CSV file
    cnn_df = pd.DataFrame({'CNN Classification': predicted_labels})
    cnn_df.to_csv('cnn_res.csv', index=False)
    print('CNN classification results saved to cnn_res.csv')

    # Display operation end label in green
    begin_label.config(text="CNN Classification operation Complete", fg="green")
    root.update()
...
def apply_random_forest_classification():
    ...
    # Write classification results to a CSV file
    random_forest_df = pd.DataFrame({'Random_Forest_Classification': predicted_classification, 'Precision': [precision]*len(predicted_classification), 'Recall': [recall]*len(predicted_classification), 'F1 Score': [f1]*len(predicted_classification)})
...
def apply_fuzzification():
    ...
    # Display the fuzzification results and metrics in the text box
    fuzzification_text = '\n\n'.join(fuzzification_results)
    fuzzification_text += f"\nPrecision: {precision}\nRecall: {recall}\nF1 Score: {f1}\n\n"
    text_area.delete(1.0, tk.END)
    text_area.insert(tk.END, fuzzification_text)

    # Write fuzzification results to a CSV file
    fuzzification_df = pd.DataFrame({'Fuzzy Sentiment': fuzzification_results, 'Precision': [precision]*len(fuzzification_results), 'Recall': [recall]*len(fuzzification_results), 'F1 Score': [f1]*len(fuzzification_results)})
...
def hybridization():
    ...
    cnn_df = pd.read_csv('cnn_classification_output.csv')
    ...
    selected_df = pd.read_csv('sentiment_analysis_output.csv')
    selected_text = selected_df.apply(lambda row: ' '.join([str(row[col]) for col in selected_df.columns]), axis=1)
    true_labels = selected_df['Label']

    # Compare classifications and create a hybrid result
    hybrid_classifications = []
    labels = []
    for cnn_class, rf_class, true_label, selected_text in zip(cnn_classifications, rf_classifications, true_labels, selected_text):
        if cnn_class == rf_class:
            # If both classifications are the same, use that classification
            hybrid_label = cnn_class
        else:
            # If classifications differ, analyze the selected text
            hybrid_label = analyze_selected_text(selected_text)

        # Calculate precision, recall, and F1 score
        if true_label == hybrid_label:
            labels.append(0)  # True negative or true positive
        else:
            labels.append(1)  # False negative or false positive

        genuine_fake_class = classify_genuine_fake(selected_text)
        hybrid_classifications.append(
            f"Text: {selected_text}\nHybrid Classification: {hybrid_label}\nGenuine/Fake: {genuine_fake_class}\n\n"
        )

    # Calculate precision, recall, and F1 score
    precision, recall, f1 = calculate_metrics(labels, [1] * len(labels))

    # Display the hybrid classification results and metrics in the text box
    hybridization_text = '\n\n'.join(hybrid_classifications)
    hybridization_text += f"\nPrecision: {precision}\nRecall: {recall}\nF1 Score: {f1}\n\n"
    text_area.delete(1.0, tk.END)
    text_area.insert(tk.END, hybridization_text)

    # Write hybrid classification results to a CSV file
    hybrid_df = pd.DataFrame({'Hybrid Classification': hybrid_classifications, 'Precision': [precision]*len(hybrid_classifications), 'Recall': [recall]*len(hybrid_classifications), 'F1 Score': [f1]*len(hybrid_classifications)})
...
btn11 = Button(root, text="12. HYBRIDIZATION", fg="white", bg="blue", activebackground="red", command=hybridization)
