...
def apply_stopword_removal():
    ...
    try:
        ...
        text_area.delete(1.0, tk.END)
        text_area.insert(tk.INSERT, "Filtered output file is empty. Please run Filtration first.")
        return
...
def apply_fuzzification():
    ...
    sentiment['negative'] = fuzz.trimf(sentiment.universe, [-1, -1, -0.05])
    sentiment['neutral'] = fuzz.trimf(sentiment.universe, [-0.05, 0, 0.05])
    sentiment['positive'] = fuzz.trimf(sentiment.universe, [0.05, 1, 1])

    # Define output variable and membership functions
    classification = ctrl.Consequent(np.arange(-1, 1.01, 0.01), 'classification')
    classification['negative'] = fuzz.trimf(classification.universe, [-1, -1, -0.05])
    classification['neutral'] = fuzz.trimf(classification.universe, [-0.05, 0, 0.05])
    classification['positive'] = fuzz.trimf(classification.universe, [0.05, 1, 1])
    ...
    print("Antecedents:", sentiment_classification.ctrl.antecedents)
    print("Consequents:", sentiment_classification.ctrl.consequents)
    ...
    for text_id, selected_text in zip(text_id, selected_text):
        ...
        if sentiment_class <= -0.05:
            classification_label = 'Negative'
        elif sentiment_class >= 0.05:
...
def analyze_selected_text(cnn_class, rf_class, selected_text):
    sid = SentimentIntensityAnalyzer()
    ...
    #Calculate variance
    variance = int(cnn_class != rf_class)
    return variance
...
def hybridization():
    ...
    for cnn_class, rf_class, text_id, selected_text in zip(cnn_classifications, rf_classifications, text_id, selected_text):
        if cnn_class == rf_class:
            ...
            correct_class = analyze_selected_text(cnn_class, rf_class, selected_text)
            variance = analyze_selected_text(cnn_class, rf_class, selected_text)
...
btn6.grid(row= 8, column= 0, padx=10, pady=10, sticky='nesw')
...
btn7.grid(row= 9, column= 0, padx=10, pady=10, sticky='nesw')
...
btn8.grid(row= 10, column= 0, padx=10, pady=10, sticky='nesw')
...
btn9.grid(row=11, column=0, padx=10, pady=10, sticky='nesw')
...
btn10.grid(row=12, column=0, padx=10, pady=10, sticky='nesw')
...
btn11.grid(row=13, column=0, padx=10, pady=10, sticky='nesw')
