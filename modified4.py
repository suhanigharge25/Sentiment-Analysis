...
def apply_random_forest_classification():
    ...
    predicted_classification = []
    for text_id, selected_text in zip(text_id, selected_text):
        sequence = tokenizer.texts_to_sequences([selected_text])
        padded_sequence = pad_sequences(sequence, maxlen=max_len, truncating='post', padding='post')
        prediction = random_forest_model.predict(padded_sequence)[0]
        classification = label_encoder.classes_[prediction]
        predicted_classification.append(
...
def apply_fuzzification():
    # Display operation begin label in red
    begin_label.config(text="Performing Fuzzy Logic Classification", fg="red")
    root.update()
    ...
    # Apply fuzzy logic classification to each row separately
    sid = SentimentIntensityAnalyzer()
    fuzzy_logic_classification = []
    for text_id, selected_text in zip(text_id, selected_text):
        fuzzy_score = sid.polarity_scores(str(selected_text)) if pd.notna(selected_text) else {}
        if fuzzy_score >= 0.07:
           classification = 'Positive'
        elif fuzzy_score <= 0.03:
           classification = 'Negative'
        else:
           classification = 'Neutral'
           
        fuzzy_logic_classification.append(
            f"Text: {selected_text}\nFuzzy Logic Classification: {classification}\n\n"
        )

    # Display the classification results in the text box
    classification_text = '\n\n'.join(fuzzy_logic_classification)
    text_area.delete(1.0, tk.END)
    text_area.insert(tk.END, classification_text)

    # Write classification results to a CSV file
    fuzzy_logic_df = pd.DataFrame({'Fuzzy_Logic_Classification': fuzzy_logic_classification})
    fuzzy_logic_df.to_csv('fuzzy_logic_classification_output.csv', index=False)
    print('Fuzzy Logic classification results saved to fuzzy_logic_classification_output.csv')

    # Display operation end label in green
    begin_label.config(text="Fuzzy Logic Classification operation Complete", fg="green")
    root.update()

    total_records = len(fuzzy_logic_df)
    increment = 100 / total_records
