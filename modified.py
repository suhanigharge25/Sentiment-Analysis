...
def apply_review_classification():
    ...
    try:
        selected_df = pd.read_csv('selected_output.csv')
        selected_text = selected_df.apply(lambda row: ' '.join([str(row[col]) for col in selected_df.columns]), axis=1)
        text_ids = selected_df['Id'].astype(str)
    except pd.errors.EmptyDataError:
        text_area.delete(1.0, tk.END)
        text_area.insert(tk.END, "Selected output file is empty. Please run Apply Selection first.")
        return
   ...
   #
    ...
    sid = SentimentIntensityAnalyzer()
    for selected_text in zip(selected_text):
...
def apply_cnn():
    ...
    train_texts, test_texts = train_test_split(selected_text, test_size=0.2, random_state=42)
    ...
    train_labels = np.random.choice(classes, size=len(train_texts))
    test_labels = np.random.choice(classes, size=len(test_texts))
    ...
    model.fit(train_padded, train_labels_encoded, epochs=5, validation_data=(test_padded, test_labels_encoded))

    # Predict the classes for the test set
    predictions = model.predict(test_padded)
    #predicted_labels = np.argmax(predictions, axis=1)
    #predicted_classes = label_encoder.inverse_transform(predicted_labels)
    predicted_classes = [classes[np.argmax(probabilities)] for probabilities in predictions]
    # Evaluate the model on the test set
    test_loss, test_accuracy = model.evaluate(test_padded, test_labels_encoded)

    # Display accuracy in the text area
    accuracy_text = f"CNN Classification Accuracy on Selected Data: {test_accuracy * 100:.2f}%"
    text_area.delete(1.0, tk.END)
    text_area.insert(tk.END, accuracy_text)

    # Save results to a CSV file
    cnn_results_df = pd.DataFrame({'Text': test_texts, 'Actual Classification': test_labels, 'Predicted Classification': predicted_classes})
    cnn_results_df.to_csv('cnn_classification_on_selected_data_output.csv', index=False)
    print('CNN classification results on selected data saved to cnn_classification_on_selected_data_output.csv')

    # Display operation end label in green
    begin_label.config(text="CNN Classification on Selected Data operation Complete", fg="green")
    root.update()
    
    total_records = len(cnn_results_df)
    increment = 100 / total_records
...
btn8= Button(root, text="8. CNN", fg="white", bg="blue", activebackground="red", command=apply_cnn)
btn8.grid(row= 11, column= 0, padx=10, pady=10, sticky='nesw')

btn8= Button(root, text="Exit", fg="white", bg="blue", activebackground="red", command=Close)
btn8.grid(row= 12, column= 0, padx=10, pady=10, sticky='nesw')
